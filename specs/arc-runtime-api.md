...
Arc selection: (1) resume each candidate's trigger consultation with the accepted report data — answered work stays pinned per candidate across the brief chain, candidates whose consultation already reached a terminal outcome keep it without re-running, and a `preferredMatch` sent on an earlier chain report is retained; (2) if the effective `preferredMatch` is currently matchable, select it; (3) otherwise, if exactly one arc is matchable, auto-select it; (4) otherwise return another trigger brief. A fresh `startTrigger(...)` starts new consultations for every candidate; consultation state does not survive a process restart.

When trigger report validation fails, `progressTrigger(...)` returns a new `TriggerBrief` with `issues` describing the rejection. For issues with `kind: "invalid-report"`, the runtime rejects the report as a whole, applies no changes, and re-yields the same trigger frontier. For issues with `kind: "invalid-item"`, the runtime applies the valid subset of reported results and re-yields the rejected work items.
...
When multiple arcs are matchable and no `preferredMatch` selects one, the runtime returns a `TriggerBrief` with `issues` containing `kind:"ambiguous-match"` and expects a later `TriggerReport` to name one specific `preferredMatch`.
